...
(function(){
  ...
  style.textContent=
    ...
    /* macOS gets a denser, quieter control dock: it sits on a dedicated native
       window rather than competing with browser chrome, so its materials and
       spacing can echo the window's editorial surface. */
    "html.shell-mac .nbar{left:18px;right:18px;height:44px;padding:0 10px;gap:4px;border-radius:13px;border-color:rgba(255,255,255,.12);background:linear-gradient(180deg,rgba(31,34,40,.82),rgba(18,20,25,.76));box-shadow:0 12px 34px rgba(0,0,0,.42),inset 0 1px 0 rgba(255,255,255,.08)}"+
    "html.shell-mac .nbar button{width:30px;height:30px;color:rgba(255,255,255,.93)}html.shell-mac .nbar button:hover{background:rgba(255,255,255,.13)}html.shell-mac .nbar svg{width:17px;height:17px}html.shell-mac .ntrack{height:24px;padding:0 11px}html.shell-mac .nrail{height:3px;background:rgba(255,255,255,.24)}html.shell-mac .nfill{background:linear-gradient(90deg,#ff6a00,#ffaf6e)}html.shell-mac .nknob{width:10px;height:10px;margin:-5px 0 0 -5px;background:#fff;box-shadow:0 1px 5px rgba(0,0,0,.7)}html.shell-mac .nlive{padding:0 7px 0 5px;font-size:10px;font-weight:700;letter-spacing:.12em}html.shell-mac .nlive i{width:5px;height:5px;box-shadow:0 0 0 3px rgba(39,209,124,.10)}"+
